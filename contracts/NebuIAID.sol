...
contract EIP712 {
    ...
    bytes32 constant NEBUIA_TYPEHASH =
        keccak256(
            "NebuIA_Identity(Issuer from,string title,string[] contents)Issuer(bytes32 name,address wallet)"
        );
    ...
    function hash(NebuIA_Identity memory ip) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    NEBUIA_TYPEHASH,
    ...
    function createIdentity(string memory title, string[] memory contents)
        internal
        pure
        returns (NebuIA_Identity memory)
    {
        NebuIA_Identity memory ip = NebuIA_Identity({
            from: Issuer({
                name: "NebuIA",
                wallet: 0xDeaDbeefdEAdbeefdEadbEEFdeadbeEFdEaDbeeF // repalce by nebuia wallet
            }),
            title: title,
            contents: contents
        });
    ...
    function recoverSigner(
        ...
        NebuIA_Identity memory identity = createIdentity(_title, _contents);
    ...
    function sign(NebuIA_Identity memory identity) internal view returns (bytes32) {
        bytes32 digest = keccak256(
            abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, hash(identity))
        );
...
contract NebuIAID is ERC721, EIP712, Ownable {
